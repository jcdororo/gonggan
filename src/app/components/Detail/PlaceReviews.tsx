...
import ModalPolice from "../Police/ModalPolice";
...
export default function PlaceReviews({ _id }: PlaceProps) {
  ...
  const { data: reviews } = useQuery<ReviewType[]>(`review-${_id}`, getReviews, {
    enabled: !!_id,
  ...
  // 리뷰 모달
  const onClick = (e: React.MouseEvent<HTMLDivElement>, review?: ReviewType) => {
    e.preventDefault();
    ...
    <>
      <div className="flex justify-between px-8 pt-10 pb-5 ">
        <div className="flex gap-3">
          ...
          <div className="mt-3">{totalStars === 0 ? "" : totalStars && reviews && <Star star={Math.floor(totalStars / reviews.length)} />}</div>
          <div className="mt-3">리뷰 {reviews?.length}개</div>
        </div>
        {status == "authenticated" && (
          <div className="m-2 pt-[6px] cursor-pointer" id="write" onClick={(e) => onClick(e)}>
            {/* 리뷰를 작성한 이력이 있으면 작성하기 버튼 안 보임 */}
            {reviews?.some((review) => review.writerid === userData.user?._id) ? "" : "리뷰 작성하기"}
          </div>
        )}
      </div>
              ...
              <div className="m-auto w-[89%] p-5 border border-solid border-black rounded-md mb-5">
                <div className="flex justify-between mb-3">
                  <div className="flex gap-4">
                    <div className="flex items-center">
                      <Image className="rounded-full h-11 w-11" src={review.writerpic ? review.writerpic : "/logo.png"} width={400} height={400} alt="아이콘" />
                    </div>
                    <div className="my-auto">
                      <div className="font-bold">{review.writernickname}</div>
                ...
                {userData && userData.user?._id == review.writerid ? (
                  <div className="flex justify-end gap-2 text-xs">
                    <div id="modify" onClick={(e) => onClick(e, review)} className="flex justify-end cursor-pointer hover:opacity-80">
                      수정하기
                    </div>
                    <div id="delete" onClick={(e) => onClick(e, review)} className="flex justify-end cursor-pointer hover:opacity-80">
                      삭제하기
                    </div>
                  </div>
                ) : (
                  <div id="police" onClick={(e) => onClick(e, review)} className="flex justify-end text-xs cursor-pointer hover:opacity-80">
                    신고하기
                  </div>
                )}
              </div>
      ...
      {/* 모달 */}
      {reviewWrite ? <ReviewShape onClick={onClick} _id={_id} userData={userData} type="write" /> : ""}
      {reviewModify ? <ReviewShape onClick={onClick} review={review} _id={_id} userData={userData} type="modify" /> : ""}
      {reviewDelete ? <ModalDelete targetContent={review} handleDelete={onClick} /> : ""}
      {policeWrite ? <ModalPolice targetContent={review} handleWrite={onClick} /> : ""}
    </>
  );
}
