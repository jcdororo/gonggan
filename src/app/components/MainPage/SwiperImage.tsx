...
export default function SwiperImage({ place }: SwiperImageProps) {
  ...
  useEffect(() => {
    const getPlace = async () => {
      const { data } = await axios.get(`/api/place/getPicture?_id=${place._id}`);
      setPictures(data);
    };
    getPlace();
  }, []);
        ...
        {pictures?.map((picture) => (
          <SwiperSlide key={picture._id.toString()}>
            <div className="relative w-full h-[350px]">
              <Image src={picture.url} alt="장소 이미지" layout={"fill"} priority className="rounded-md cursor-pointer" onClick={() => router.push(`/places/${picture.place_id}`)} />
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
