...
import SpinWhite from "../Spin/SpinWhite";
                ...
                <tr key={x._id.toString()} className="h-10">
                  <td>
                    <div className="flex justify-center items-center">
                      <div className="flex justify-center items-center bg-sygnature-brown rounded-2xl h-8 w-16 text-sm text-white cursor-pointer" onClick={(e) => handleClick(e, x._id.toString(), i, x.role)}>
                        {isLoading ? <SpinWhite /> : x.role}
                      </div>
                    </div>
                  </td>
                  <td className="py-4 pl-10">{x._id.toString()}</td>
                  <td className="py-2 hover:underline cursor-pointer text-center">{x.nickname}</td>
                  <td className="py-2 text-center">{x.method}</td>
                </tr>
              ))}
            </tbody>
          </table>
